		...
		<el-table :data="tableData.tableList" :header-cell-style="tableheader" v-loading="tableData.loading " element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
			...
			<el-table-column label="生产计划" width="230">
				<template slot-scope="scope">
					...
					<el-button type="text" size="mini" :disabled="scope.row.task_number>=scope.row.number?true:false" @click="ParstlistOkClick(scope.row)" style="height: 32px;margin-left: 10px;">生产</el-button>
	...
	export default {
		...
		methods: {
			getParstlist() {
				...
				that.axios({
					method: 'GET',
					url: that.url + '/api/v1/supplier/order-product-detail',
					data: '',
					params: {
                ...
                if (row.num > 0) {
                    that.$msgbox({
                        ...
                        var params = {
                            ...
                            order_detail_id: row.id,	    						// Parstlist_detail_id			string	是	订单详情ID
                            ...
                            Parstlist_id: row.Parstlist_id,                                 // Parstlist_id	                string	是	订单ID
			...
			downAll() {
				...
				if (supplier_id) {
					...
					location.href = this.url + '/api/v1/supplier/export-Parstlist?supplier_id=' + supplier_id + '&batch=' + this.batchs_ + '&name=' + this.search_name;
					// console.log(this.url + '/api/v1/supplier/export-Parstlist?supplier_id=' + supplier_id + '&batch=' + this.batchs_ + '&name=' + this.search_name)
				} else {
					this.$message({
